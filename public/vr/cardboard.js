 ...
 * 4. **A layout that doesn't depend on the OS ever going landscape.** A real
 *    two-lens viewer wants two circles side by side no matter what, but nothing
 *    stops main.js from asking for `setStacked(true)` instead when the real
 *    screen stays portrait — same two eyes, same barrel correction, just drawn
 *    one above the other so the picture still fills the screen it actually has.
 *    Each eye is still rendered into a landscape-shaped slice of an
 *    intermediate target either way — that's what a lens wants to see — only
 *    the final warp pass's on-screen destination for each half changes.
 */
...
export class Cardboard {
  ...
  constructor(renderer, camera, { dwellMs = 850 } = {}) {
    ...
    // True once the real screen stayed portrait and the two eyes are being
    // drawn one above the other to fit it, rather than side by side. A real
    // two-lens viewer was never going to line up with that regardless, so
    // unlike a rotation there is no "wrong direction" to flip.
    this.stacked = false;
  ...
  /**
   * (Re)builds the two destination quads for the warp pass to match
   * `this.stacked`. Cheap enough to call on every mode change — this only
   * runs when the real viewport crosses the portrait/landscape threshold,
   * not per frame — and simplest to just rebuild rather than mutate in place.
   */
  _buildEyeQuads() {
    for (const quad of this.eyeQuads) {
      this.warpScene.remove(quad);
    ...
    for (const [index, offset] of [[0, 0.0], [1, 0.5]]) {
      const material = new THREE.ShaderMaterial({
        ...
        depthWrite: false,
        uniforms: {
          tEyes: { value: this.target ? this.target.texture : null },
      ...
      // position straight through, so these are already in NDC. Side by side
      // ordinarily — the left half of the screen samples the left half of
      // the intermediate target, offset 0.0, and likewise for the right.
      // Stacked turns that 90°: same two source halves, now the top and
      // bottom of the screen instead, so the sampling offsets are untouched
      // and only the destination shape moves.
      const geometry = this.stacked ? new THREE.PlaneGeometry(2, 1) : new THREE.PlaneGeometry(1, 2);
      geometry.translate(
        this.stacked ? 0 : (index === 0 ? -0.5 : 0.5),
        this.stacked ? (index === 0 ? 0.5 : -0.5) : 0,
        0,
      );
  ...
  /**
   * @param {boolean} stacked Draw the two eyes one above the other instead
   *   of side by side, for when the real screen won't turn to landscape —
   *   rotation lock, or iOS never honouring screen.orientation.lock() at
   *   all. A real two-lens viewer expects side by side either way, so this
   *   is only worth it without one, or with a viewer built for it.
   */
  setStacked(stacked) {
  ...
  /**
   * The intermediate target the two eyes render into is deliberately *not*
   * always the same shape as the real screen. A lens wants a landscape-shaped
   * image regardless of how the final warp pass lays the result out, so when
   * stacked mode has turned the real (portrait) drawing buffer's dimensions
   * into the destination geometry instead, this keeps the render itself
   * landscape-shaped by swapping right back.
   */
  _resizeTarget() {
    const size = this.renderer.getDrawingBufferSize(new THREE.Vector2());
    let width = Math.max(2, size.x);
    let height = Math.max(2, size.y);
    if (this.stacked) { const t = width; width = height; height = t; }
    if (this.target) {
  ...
  render(scene) {
    ...
    // The final pass draws to the real screen, whatever shape that actually
    // is — not necessarily the target's shape above, now that stacked mode
    // can make those two diverge on purpose.
    const screenSize = renderer.getDrawingBufferSize(this._screenSize);
